...
\right)
$$ {#eq-ion-acoustic-dispersion-relation}
...
$$
で与えられることを示し，
Langmuir波については長波長近似 $k \lambda_{De} \sim k \lambda_{Di} \ll 1$ で，
$\omega^2 \sim \omega_{pe}^2$ となることから
$| \tilde{n}_i/\tilde{n}_e | \ll 1$ を，
イオン音波については @eq-ion-acoustic-dispersion-relation を用いて
$| \tilde{n}_i/\tilde{n}_e | \sim 1$ となることをそれぞれ示せ．
:::
...
- $\tilde{E}_R \neq 0, \tilde{E}_L = 0$ のとき
  ...
  $$
  で与えられる．すなわち波は円偏光で$E_y$は$E_x$に対して位相が$\pi/2$だけ遅れている．
...
- $\tilde{E}_R = 0, \tilde{E}_L \neq 0$ のとき
  ...
  \tilde{E}_{y} (t) = A \exp \left[ - i \left(\omega t + \frac{\pi}{2} \right) \right],
  $$
  で与えられる．すなわち波は円偏光で，$E_y$は$E_x$に対して位相が$\pi/2$だけ進んでいる．
...
::: {#exr-wave-reflection}
1. 磁場の影響が無視できるとき，プラズマ中を伝播する線形の電磁波は以下の形の波動方程式（Klein-Gordon方程式）によって記述される．
$$
...
@fig-whistler-helliwell に地上の受信機で観測されたホイッスラー波の観測結果を示す．このようなホイッスラー波は受信局の逆側の半球における雷放電に伴って放射され，磁力線に沿って伝播方向してきたものと考えられる．ホイッスラー波の持つ分散性（高周波成分ほど群速度が大きい）のため高周波の成分が先に到達し，可聴域の周波数で笛の音のように聞こえるためホイッスラー波と呼ばれるようになった．

![ホイッスラー波の観測 [@GurnettBhattacharjee2017]](figures/whistler_helliwell.png){width=100% #fig-whistler-helliwell}
...
これを測定することで，GPS電波の伝播経路に沿った電離層の全電子数（Total Electron Content）を求めることができる．電離層の文脈では慣習的にDMの単位として $1 \, {\rm TEC} = 10^{16} \, {\rm m}^{-2}$ なる単位が使われる．
日本上空での典型的な全電子数は $1-10 \, {\rm TEC}$ 程度となることが知られており，GPSの周波数（1.2 GHzおよび1.5 GHz）を使うと，これに対応する典型的な時間遅れが$\Delta t \sim 1-10 \, {\rm ns}$程度となることが分かる．これに光速をかけて距離に換算した値（数m程度）が電離層による遅延を起源とするGPS測位誤差となる．（実際には2周波の観測を用いることでこの誤差は補正することができる．）  
